...
def namespace_control():
    ...
    global KAKFA_CONTROL
    global is_kafka
    global kafka_IP
    global kafka_port
    global KAFKA_SWITCH

    consumer = create_consumer(kafka_IP,kafka_port,KAKFA_CONTROL)
    while True:
        for i, message in enumerate(consumer):
            ...
            try:
                ...
                the_path = msg['path']
                ns_path = f"/var/run/netns/{the_path}"
                print(f"Switching to namespace: {the_path}")
                try:
                    ...
                    os.close(fd)
                    print(f"Successfully switched to namespace: {the_path}")
...
def carla26g():
    ...
    while True:
        ...
        my_lock = threading.Lock()
        with my_lock:
            to_change = KAFKA_SWITCH
        if to_change is True:
            producer.close()
            producer = create_producer(broker_url,broker_port)
            print("Connected to new namespace")
            my_lock = threading.Lock()
            with my_lock:
                KAFKA_SWITCH = False
        global tracklets
...
def main():
    ...
    try:
        ...
        if is_kafka is True:
            ...
            print(f"Tracklets will be saved to file {kafka_file}")
        ...
        return_tensors = []
        # Threading and MQTT
        from_6G = threading.Thread(target=namespace_control)
