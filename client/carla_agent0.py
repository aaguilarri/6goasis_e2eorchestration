...
class BasicSynchronousClient(object):
    ...
    def set_image(weak_self, img):
        ...
        if self.capture:
            ...
            image_data = np.reshape(image_data, (540, 960, 4))
    ...
    def get_object_distance(self, box, tipo):
        ...
        print(f"depth is {type(depth)},{depth.shape}")
        #vals = 
        ...
        print(vals)
        val = np.mean(vals)
        #val = 10 * (vals[2] + vals[1] * 256 + vals[0] * 256 * 256) / (256 * 256 * 256 - 1)
        print(val)
        #inv_cal = self.inv_cal.cpu()
        cali = self.calibration
        obj_x = val*ptx*cali[0,2]/cali[0,0]
        obj_y = val*pty*cali[1,2]/cali[1,1]
        obj_z = val
        my_tensor = torch.tensor([ptx, pty, val]).to(torch.double)
        print(my_tensor)
            ...
            while True:
                ...
                if t1 - t0 > 1:
                    ...
                    try:
                        ...
                        print(f"res is {type(res)}, {res.shape}")
